...
const TodayCard = ({ tasks , deleteTask, complete, setEditingTask}) => {
  ...
  return (
    <div className="flex flex-col border-2 border-gray-200 rounded shadow-sm h-auto md:h-128">
      {tasks.map((task) => (
        ...
        <div key={task.id} className="flex  md:flex-row md:items-center border-2 border-gray-200 p-3 md:p-4 rounded m-2 shadow-sm">
          ...
          <p className="font-medium text-sm md:text-base flex-1 text-left">{task.text}</p>
          ...
          <div className='flex items-center gap-4'>
            <div className="flex items-center gap-4 w-[220px] justify-start">
              ...
              <span className={`px-3 py-1 rounded-full text-xs md:text-sm font-semibold ${priorityColors[task.priority]}`}>{task.priority}</span>
            </div> 
            <button 
